...
fhat_train_lda <- as.integer(predict(lda_fit, newdata = train_set, method = "plug-in")$class)
train_error_lda <- mean(fhat_train_lda != ytrain) #training error
train_error_lda #Performing super bad

#predictions using test set
fhat_test_lda <- as.integer(predict(lda_fit, newdata = test_set, method = "plug-in")$class)
test_error_lda <- mean(fhat_test_lda != ytest) #test error
test_error_lda #Performing super bad
```
...
#predictions using training set
fhat_train_lda2 <- as.integer(predict(lda_fit2, newdata = train_set, method = "plug-in")$class)
train_error_lda2 <- mean(fhat_train_lda2 != ytrain)
train_error_lda2 #Performing super bad

#predictions using test set
fhat_test_lda2 <- as.integer(predict(lda_fit2, newdata = test_set, method = "plug-in")$class)
test_error_lda2 <- mean(fhat_test_lda2 != ytest)
test_error_lda2 #Performing super bad
```
...
#predictions using training set
fhat_train_qda <- as.integer(predict(qda_fit, newdata = train_set, method = "plug-in")$class)
train_error_qda <- mean(fhat_train_qda != ytrain)
train_error_qda #Performing super bad

#predictions using test set
fhat_test_qda <- as.integer(predict(qda_fit, newdata = test_set, method = "plug-in")$class)
test_error_qda <- mean(fhat_test_qda != ytest)
test_error_qda #Performing super bad
```
...
#predictions using training set
fhat_train_qda2 <- as.integer(predict(qda_fit2, newdata = train_set, method = "plug-in")$class)
train_error_qda2 <- mean(fhat_train_qda2 != ytrain)
train_error_qda2 #Performing super bad

#predictions using test set
fhat_test_qda2 <- as.integer(predict(qda_fit2, newdata = test_set, method = "plug-in")$class)
test_error_qda2 <- mean(fhat_test_qda2 != ytest)
test_error_qda2 #Performing super bad
```
...
for (i in 1:length(lambdas)){
  ...
  #fitting boosting model
   ...
   #training error
   fhat_train_boost <- as.integer(predict(boost_fit, train_set, n.trees = 500))
   train_error_boost[i] <- mean(fhat_train_boost != ytrain)

   #test error
   fhat_test_boost <- as.integer(predict(boost_fit, test_set, n.trees = 500))
...
#training error
fhat_train_multinom <- as.integer(predict(multinom_fit, train_set))
train_error_multinom <- mean(fhat_train_multinom != ytrain)
train_error_multinom 

#test error
fhat_test_multinom <- as.integer(predict(multinom_fit, test_set))
...
#training error
fhat_train_multinom2 <- as.integer(predict(multinom_fit2, train_set))
train_error_multinom2 <- mean(fhat_train_multinom2 != ytrain)
train_error_multinom2 

#test error
fhat_test_multinom2 <- as.integer(predict(multinom_fit2, test_set))
...
#training error
fhat_train_lasso <- as.integer(predict(lasso_fit, s = bestlambda, newx = xtrain))
train_error_lasso <- mean(fhat_train_lasso != ytrain)
train_error_lasso

#test error
fhat_test_lasso <- as.integer(predict(lasso_fit, s = bestlambda, newx = xtest))
...
fhat_train_qda_pc <- as.integer(predict(qda_fit_pc, newdata = pc_combined, method = "plug-in")$class)
train_error_pc <- mean(fhat_train_qda_pc != pc_combined$Diabetes_012)
train_error_pc #still extremely high. 
```
...
#The fhat tests are all equal to 3. idk what went wrong. 
fhat_test_qda_pc <- as.integer(predict(qda_fit_pc, newdata = as.data.frame(new_test), method = "plug-in")$class)
